...
func _ready() -> void:
	super._ready()
	actions.configure_cast(&"heal", heal_cooldown, 0.30, _release_heal)

func _valid_ally(ally: Unit) -> bool:
	return is_instance_valid(ally) and ally != self and ally.is_inside_tree() and not ally.is_queued_for_deletion() and ally.is_targetable() and ally.team == team
